...
void AodvAttackerRSA::initialize(int stage)
{
...
void AodvAttackerRSA::handleMessageWhenUp(cMessage *msg)
{
...
void AodvAttackerRSA::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void AodvAttackerRSA::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result AodvAttackerRSA::ensureRouteForDatagram(Packet *datagram)
{
...
AodvAttackerRSA::AodvAttackerRSA()
{
}

bool AodvAttackerRSA::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void AodvAttackerRSA::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address AodvAttackerRSA::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void AodvAttackerRSA::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void AodvAttackerRSA::sendRREQ(const Ptr<RreqSec>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AodvAttackerRSA::sendRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<RreqSec> AodvAttackerRSA::createRREQ(const L3Address& destAddr)
{
...
const Ptr<RrepSec> AodvAttackerRSA::createRREP(const Ptr<RreqSec>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<RrepSec> AodvAttackerRSA::createGratuitousRREP(const Ptr<RreqSec>& rreq, IRoute *originatorRoute)
{
...
void AodvAttackerRSA::handleRREP(const Ptr<RrepSec>& rrep, const L3Address& sourceAddr)
{
...
void AodvAttackerRSA::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void AodvAttackerRSA::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void AodvAttackerRSA::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void AodvAttackerRSA::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void AodvAttackerRSA::socketClosed(UdpSocket *socket)
{
...
void AodvAttackerRSA::handleRREQ(const Ptr<RreqSec>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *AodvAttackerRSA::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void AodvAttackerRSA::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void AodvAttackerRSA::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> AodvAttackerRSA::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void AodvAttackerRSA::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void AodvAttackerRSA::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void AodvAttackerRSA::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void AodvAttackerRSA::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void AodvAttackerRSA::clearState()
{
...
void AodvAttackerRSA::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void AodvAttackerRSA::forwardRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AodvAttackerRSA::forwardRREQ(const Ptr<RreqSec>& rreq, unsigned int timeToLive)
{
...
void AodvAttackerRSA::completeRouteDiscovery(const L3Address& target)
{
...
void AodvAttackerRSA::sendGRREP(const Ptr<RrepSec>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<RrepSec> AodvAttackerRSA::createHelloMessage()
{
...
void AodvAttackerRSA::sendHelloMessagesIfNeeded()
{
...
void AodvAttackerRSA::handleHelloMessage(const Ptr<RrepSec>& helloMessage)
{
...
void AodvAttackerRSA::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void AodvAttackerRSA::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result AodvAttackerRSA::datagramForwardHook(Packet *datagram)
{
...
void AodvAttackerRSA::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void AodvAttackerRSA::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool AodvAttackerRSA::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> AodvAttackerRSA::createRREPACK()
{
...
void AodvAttackerRSA::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void AodvAttackerRSA::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void AodvAttackerRSA::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void AodvAttackerRSA::handleBlackListTimer()
{
