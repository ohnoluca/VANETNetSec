...
void AodvAttacker::initialize(int stage)
{
...
void AodvAttacker::handleMessageWhenUp(cMessage *msg)
{
...
void AodvAttacker::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void AodvAttacker::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result AodvAttacker::ensureRouteForDatagram(Packet *datagram)
{
...
AodvAttacker::AodvAttacker()
{
}

bool AodvAttacker::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void AodvAttacker::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address AodvAttacker::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void AodvAttacker::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void AodvAttacker::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AodvAttacker::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> AodvAttacker::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> AodvAttacker::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> AodvAttacker::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void AodvAttacker::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
...
void AodvAttacker::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void AodvAttacker::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void AodvAttacker::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void AodvAttacker::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void AodvAttacker::socketClosed(UdpSocket *socket)
{
...
void AodvAttacker::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *AodvAttacker::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void AodvAttacker::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void AodvAttacker::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> AodvAttacker::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void AodvAttacker::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void AodvAttacker::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void AodvAttacker::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void AodvAttacker::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void AodvAttacker::clearState()
{
...
void AodvAttacker::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void AodvAttacker::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AodvAttacker::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void AodvAttacker::completeRouteDiscovery(const L3Address& target)
{
...
void AodvAttacker::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> AodvAttacker::createHelloMessage()
{
...
void AodvAttacker::sendHelloMessagesIfNeeded()
{
...
void AodvAttacker::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void AodvAttacker::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void AodvAttacker::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result AodvAttacker::datagramForwardHook(Packet *datagram)
{
...
void AodvAttacker::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void AodvAttacker::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool AodvAttacker::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> AodvAttacker::createRREPACK()
{
...
void AodvAttacker::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void AodvAttacker::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void AodvAttacker::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void AodvAttacker::handleBlackListTimer()
{
