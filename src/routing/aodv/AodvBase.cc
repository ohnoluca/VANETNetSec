...
Define_Module(AodvBase);

const int KIND_DELAYEDSEND = 100;

void AodvBase::initialize(int stage)
{
...
void AodvBase::handleMessageWhenUp(cMessage *msg)
{
...
void AodvBase::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void AodvBase::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result AodvBase::ensureRouteForDatagram(Packet *datagram)
{
...
bool AodvBase::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void AodvBase::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address AodvBase::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void AodvBase::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void AodvBase::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AodvBase::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> AodvBase::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> AodvBase::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> AodvBase::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void AodvBase::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
...
void AodvBase::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void AodvBase::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void AodvBase::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void AodvBase::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void AodvBase::socketClosed(UdpSocket *socket)
{
...
void AodvBase::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *AodvBase::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void AodvBase::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void AodvBase::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> AodvBase::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void AodvBase::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void AodvBase::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void AodvBase::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void AodvBase::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void AodvBase::clearState()
{
...
void AodvBase::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void AodvBase::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AodvBase::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void AodvBase::completeRouteDiscovery(const L3Address& target)
{
...
void AodvBase::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> AodvBase::createHelloMessage()
{
...
void AodvBase::sendHelloMessagesIfNeeded()
{
...
void AodvBase::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void AodvBase::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result AodvBase::datagramForwardHook(Packet *datagram)
{
...
void AodvBase::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void AodvBase::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool AodvBase::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> AodvBase::createRREPACK()
{
...
void AodvBase::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void AodvBase::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void AodvBase::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void AodvBase::handleBlackListTimer()
{
